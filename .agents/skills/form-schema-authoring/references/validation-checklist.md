...
Validate in this order. Failures must use stable `code` + JSON Pointer `path` + `message`.
...
- `validations[].code`: `^[A-Z][A-Z0-9_]{2,63}$` (SCREAMING_SNAKE, e.g. `FULL_NAME_REQUIRED`)
...
Never put `pattern` or extra keys inside `rules.validations[]` items — only `code`, `message`, `assert`, optional `when`.
