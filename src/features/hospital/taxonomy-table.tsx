...
interface TaxonomyTableProps {
  ...
  onRetire: (item: TaxonomyRow) => void;
  onAddFirst: () => void;
}
...
export function TaxonomyTable({
  ...
  const isNested = resource !== 'facilities';
  const columnCount = (isNested ? 5 : 4) + (canWrite ? 1 : 0);
  const parentLabel = t(
    resource === 'clinicalAreas'
      ? 'clinicalAreas.facilityLabel'
      : 'disciplines.clinicalAreaLabel',
  );
